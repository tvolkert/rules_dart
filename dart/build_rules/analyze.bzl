...
def analyze_action(ctx, dart_ctx, summary=None,
                   use_summaries=True, analysis=None, fail_on_error=True,
                   x_perf_report=None, strong=False):
  """Runs the analyzer.

  Arguments:
    ctx: The rule context.
    dart_ctx: The Dart context.
    summary: The output semantic summary file, or `None` if no semantic summary
        is desired (analyzer tests for instance).  Defaults to `None`.
  ...
  script_needed = not use_build_mode and (analysis != None or ignore_exit_code)
  ...
  if use_build_mode:
    ...
    if use_summaries and dependent_ctxs:
      ...
      analyzer_args += ["--build-summary-input=%s" % ",".join(
          [s.path for s in [dc.strong_summary for dc in dependent_ctxs] if s])]
    if summary:
  ...
  short_paths_to_analyze = [
      f.short_path for f in filter_files(dart_filetypes, dart_ctx.srcs)
  ...
  if use_build_mode:
    ...
    analyzer_args += [
        analyze_dir_files[p].path for p in short_paths_to_analyze]
